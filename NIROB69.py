...
\033[1;37;1m-----------------------------------------------
 AUTHOR        : Mohammad Jubayet Sarkar
 GITHUB         :  Nirjoynirob69k
 FACEBOOK   : MD JUBAYET SARKAR
 TOOLS     \033[1;97m : \x1b[97m\033[37;41m PREMIUM \033[0;m
...
def meyexudi():
  ...
  id = "K".join(uuid)
  try:
    httpCaht = requests.get('https://github.com/Nirjoynirob69k/NIROB69/blob/main/APPROVAL.txt').text
    if id in httpCaht:
      msg = str(os.geteuid())
      #time.sleep(0.5)
      print()
      ...
      os.system('am start https://www.facebook.com/profile.php?id=100000974981454'),approval()      
      time.sleep(1)
      meyexudi()
  except:
    sys.exit()
meyexudi()
#os.system("python NIROB69.py")
def naima():
	os.system('clear')
...
while attemps < 12345677901:
    ...
    if username == 'Nirob' and password == 'Nirob':
        print(' ')
        break
    else:
...
def crack(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r {P}[NIROB-XD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
